...
const { get, post } = require('../utils/requestPromise')
...
class BooksService {
   ...
   * @memberof Books
   */
  async createBooks({ name, auther, price }) {
    if( name == undefined || auther == undefined || price == undefined ) {
      return false
    }else {
      const data = await post('create', { name, auther, price })
  ...
  async updateBooks(ctx) {
    ...
    if( method == 'GET' ) {
      ...
      const { id, name, auther, price } = ctx.request.body
      const data = await post(`update&id=${id}`, { id, name, auther, price })
      return data
    }
  }
